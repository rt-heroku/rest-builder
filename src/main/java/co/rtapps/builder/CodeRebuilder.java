...
//	private static String rootPackage = "co.rtapps";
//	private static String entityPackage = "co.rtapps.entities";
	...
	private static boolean reDownloadTemplates = false;
	...
	public static void run() throws SQLException, TelosysToolsException, Exception {
		...
		TelosysToolsCfg ttCfg = loadToolsCfg(dir);
		
		DatabaseConfiguration databaseConfiguration = loadConfig(ttCfg);

		saveDatabaseConfiguration(ttCfg, databaseConfiguration);

		if (reDownloadTemplates )
	...
	private static TelosysToolsCfg loadToolsCfg(String dir) throws TelosysToolsException {
	...
	private static void loadGenerationTargetsAndGenerateFiles(String repoName, TelosysToolsCfg ttCfg,
		...
		System.out.println("Number of file(s) generated : " + generationTaskResult.getNumberOfFilesGenerated());
	}
	...
	public static DatabaseConfiguration loadConfig(TelosysToolsCfg ttCfg) throws URISyntaxException, TelosysToolsException {
		File file = new File(ttCfg.getDatabasesDbCfgFileAbsolutePath());
		DbConfigManager dbConfigManager = new DbConfigManager(file);
		DatabaseConfiguration dbcfg = dbConfigManager.load().getDatabaseConfiguration(0);
		return dbcfg;
	}
